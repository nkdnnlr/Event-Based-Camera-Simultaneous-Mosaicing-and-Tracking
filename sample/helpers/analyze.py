...
def score_quaternions(output_file, poses_gt, poses_tracker):
    ...
    for idx, row in poses_tracker.iterrows():
        poses_gt = poses_gt.append(
            pd.DataFrame({"t": [np.round(row["t"], 8)]}), ignore_index=True
        )
    ...
    for idx, row in poses_tracker.iterrows():
        t = np.round(row["t"], 8)
        qw, qx, qy, qz, t = poses_gt_intrapolated[
            poses_gt_intrapolated["t"] == t
        ].values[0]
        scores.loc[idx, "t"] = t
